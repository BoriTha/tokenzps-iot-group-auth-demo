...
class CryptoUtils:
    ...
    @staticmethod
    def lagrange_interpolate_at_zero_exact(points_xy):
        """
        points_xy: list of (x, y) pairs (ints or Fractions).
        Returns P(0) exactly as a Fraction, where P is the unique
        polynomial of degree < len(points_xy) through those points.
        """
        S = Fraction(0, 1)
        t = len(points_xy)
        for i in range(t):
